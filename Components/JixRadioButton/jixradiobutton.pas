...
type
  TJixRadioButton = class(TGraphicControl, jJixComponent)
  Private
...
Constructor TJixRadioButton.Create(AOwner: TComponent);
Begin
  Inherited;
...
Destructor TJixRadioButton.Destroy;
Begin
  Rounding.Free;
  StateNormal.Free;
  StateHover.Free;
...
Procedure TJixRadioButton.SetThemeName(Value: String);
Begin
  FThemeName:= Value;
End;

Function TJixRadioButton.GetThemeName: String;
Begin
  Result:= FThemeName;
End;

Procedure TJixRadioButton.SetTemplate(Temp: jTemplateCollectionItem);
Begin
...
Procedure TJixRadioButton.Click;
Begin
  if not Checked then
    Checked:= True;
End;

Procedure TJixRadioButton.RealSetText(const Value: TCaption);
Begin
  Inherited;
...
procedure TJixRadioButton.Paint;
...
Begin
  ...
  Begin
    Bmp.FillRoundRectAntialias(
      BS/4, BS/4, BS/4*3, BS/4*3,
      Rounding.RoundX, Rounding.RoundY,
      AddAlpha(NowState^.FontEx.Color, 255),
      Rounding.RoundOptions
    );
  End;
  {%EndRegion}
...
Procedure TJixRadioButton.SetChecked(Value: Boolean);
Var
  i: Integer;
Begin
  ...
  if Value then
    if Assigned(Parent) then
      For i:= 0 to Parent.ControlCount-1 do
        if Parent.Controls[i] is TJixRadioButton then
          TJixRadioButton(Parent.Controls[i]).Checked:= False;
...
Procedure TJixRadioButton.StateChanged(ASender: TObject; AData: PtrInt);
Begin
  Invalidate;
End;

Procedure TJixRadioButton.Mouseenter;
Begin
  Inherited;
...
Procedure TJixRadioButton.Mouseleave;
Begin
  Inherited;
...
Procedure TJixRadioButton.Mousedown(Button: Tmousebutton; Shift: Tshiftstate; X, Y: Integer);
Begin
  Inherited;
...
Procedure TJixRadioButton.Mouseup(Button: Tmousebutton; Shift: Tshiftstate; X, Y: Integer);
Begin
  Inherited;
...
procedure Register;
begin
  {$I jixradiobutton_icon.lrs}
  RegisterComponents('JixUI',[TJixRadioButton]);
end;
